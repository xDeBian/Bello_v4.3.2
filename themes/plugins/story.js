...
function sendMessageFromStory(id,fromCredits=0){
    ...
    var message = user_info.id+','+r_id+','+messageVal+',story'+','+storyId+','+fromCredits;
    var send = user_info.id+','+r_id+','+user_info.profile_photo+','+user_info.first_name+','+messageVal+',story,'+chatStoryUrl+','+chatStoryType;      
...
function updateCredits(uid,amount,type,reason,reward='') {
    if(type == 1){
         playAudio('coin.wav');
         user_info.credits = user_info.credits - amount;
    } else {
        user_info.credits = user_info.credits + amount;
    }
